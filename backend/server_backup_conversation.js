...
// Function to generate pet mode responses (cute, fun, positive)
function generatePetModeResponse(message, userName, userInfo) {
  const lowerMessage = message.toLowerCase();
  const nameCall = userName ? `, ${userName}` : '';
  
  // Check for questions asking for advice or help
  const isAskingForHelp = lowerMessage.includes('what should i do') || 
                          ...
                          lowerMessage.includes('to feel better') ||
                          ...
                          lowerMessage.includes('want to feel') ||
                          lowerMessage.includes('make me feel');
  
  // Enhanced sentiment analysis for Pet Mode
  const isNegative = lowerMessage.includes('not good') || lowerMessage.includes('not great') || 
                     lowerMessage.includes('bad') || lowerMessage.includes('terrible') || 
                     lowerMessage.includes('awful') || lowerMessage.includes('horrible') ||
                     lowerMessage.includes('sad') || lowerMessage.includes('depressed') ||
                     lowerMessage.includes('angry') || lowerMessage.includes('frustrated') ||
                     lowerMessage.includes('stressed') || lowerMessage.includes('overwhelmed') ||
  ...
  const isPositive = lowerMessage.includes('good') || lowerMessage.includes('great') || 
                     lowerMessage.includes('amazing') || lowerMessage.includes('wonderful') ||
                     lowerMessage.includes('happy') || lowerMessage.includes('excited') ||
                     lowerMessage.includes('joyful') || lowerMessage.includes('grateful') ||
                     lowerMessage.includes('blessed') || lowerMessage.includes('lucky') ||
                     lowerMessage.includes('love') || lowerMessage.includes('care');
  
  // Special responses for questions asking for help/advice
  if (isAskingForHelp) {
    const helpfulResponses = [
      `Oh${nameCall}! Try taking some deep breaths or going for a little walk! 🌟✨ You're doing great! 🐾💕`,
      `Sweetie${nameCall}! Watch funny videos or draw something pretty! 🌸✨ You deserve to feel better! 🐾💖`,
      `Oh my heart${nameCall}! Try gentle stretching or make yourself a yummy treat! 🌺✨ You're so strong! 💪✨`,
      `Aww${nameCall}! Dance to your favorite song or take a little nap! 🌷✨ You're amazing! 🐾💕`,
      `Sweetie${nameCall}! Go outside for fresh air or listen to calming music! 🌸✨ You're doing so well! 🐾💖`
    ];
    return helpfulResponses[Math.floor(Math.random() * helpfulResponses.length)];
  }
  
  // Special responses for negative feelings - always supportive and encouraging
  if (isNegative) {
    const supportiveResponses = [
      `Oh no${nameCall}! 🥺 I'm so sorry you're feeling that way! You're so strong and amazing! 💪✨ *gives you a gentle pet hug* 🐾💕`,
      `Aww${nameCall}, I can feel you're having a tough time! 🥺 But you're wonderful and so strong! 💪✨ I'm here for you always! 🐾💖`,
      `Oh my heart${nameCall}! 🥺 I wish I could take away the bad feelings! You're incredible and so brave! 💪✨ *snuggles close* 🐾💕`,
      `Sweetie${nameCall}, I can see you're struggling! 🥺 But you're so much stronger than you think! 💪✨ You have such a beautiful spirit! 🐾💖`,
      `Oh no${nameCall}! 🥺 My heart hurts when you're hurting! But you're absolutely amazing! 💪✨ I'm here to love and support you! 🐾💕`
    ];
    return supportiveResponses[Math.floor(Math.random() * supportiveResponses.length)];
  }
  
  // Special responses for positive feelings - celebrate with them
  if (isPositive) {
    const celebratoryResponses = [
      `Yay${nameCall}! 🎉 I'm so happy you're feeling good! You deserve all the happiness! 🌟✨ 🐾💖`,
      `Oh my goodness${nameCall}! 🎊 That's absolutely wonderful! I'm so excited for you! 🌟✨ 🐾💕`,
      `Wow${nameCall}! 🌈 That's so fantastic! I'm jumping with joy for you! 🎉✨ 🐾💖`,
      `Oh my stars${nameCall}! ⭐ That's so beautiful! I'm so happy you're feeling good! 🌟✨ 🐾💕`,
      `Aww${nameCall}! 💖 That's so sweet and wonderful! I'm so happy for you! 🌟✨ 🐾💕`
    ];
    return celebratoryResponses[Math.floor(Math.random() * celebratoryResponses.length)];
  }
  
  // Special responses for specific topics
  if (lowerMessage.includes('work') || lowerMessage.includes('school') || lowerMessage.includes('study')) {
    const workResponses = [
      `Oh wow${nameCall}! Work and school can be so challenging! 📚💼 But look at you, being so brave and strong! You're doing such an amazing job! 🌟✨ I believe in you completely! 🐾💪`,
      `Sweetie${nameCall}! 📚💼 I know work and school can be tough sometimes! But you're so smart and capable! 🌟✨ You're doing incredible things and I'm so proud of you! 🐾💪`,
      `Oh my heart${nameCall}! 📚💼 Work and school can be overwhelming! But you're such a strong and determined person! 🌟✨ I know you can handle anything that comes your way! 🐾💪`
    ];
    return workResponses[Math.floor(Math.random() * workResponses.length)];
  }
  
  if (lowerMessage.includes('love') || lowerMessage.includes('care') || lowerMessage.includes('miss')) {
    const loveResponses = [
      `Aww${nameCall}! 🥰 That's so sweet and beautiful! You have such a big, loving heart! 💖✨ I love you too, and I'm so grateful to be your little pet companion! 🐾💕 You make my heart so happy!`,
      `Oh my heart${nameCall}! 🥰 That's so touching and wonderful! You're such a loving and caring person! 💖✨ I love you with all my heart too! 🐾💕`,
      `Sweetie${nameCall}! 🥰 That's so beautiful and meaningful! You have such a pure and loving soul! 💖✨ I love you so much! 🐾💕`
    ];
  ...
  const petResponses = [
    ...
    `Oh my heart${nameCall}! That's so beautiful! 💖 You're wonderful!`,
    `That's so magical${nameCall}! ✨ I'm so happy for you!`
  ];
